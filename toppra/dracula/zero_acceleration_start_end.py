...
def InsertAtStartEndOfSpline(cspl_in, xin, cin, xend, cend, debugging=False):
    ...
    # insert new times:
    if debugging:
        print('\n{}\n{}\n{}'.format(cspl_in.x[time_step-1:time_step],xin,cspl_in.x[time_step+1:]))
    cspl_in.x = np.concatenate([ cspl_in.x[time_step-1:time_step],xin,cspl_in.x[time_step+1:] ])
    if debugging:
...
def ZeroAccelerationAtStartAndEnd(cspl, debugging = False):
    ...
    xstart, cstart = ThreeSegmentSpline(Ps, Vs, As, Pe, Ve, Ae, deltaTimeStart, debugging = debugging)
    ...
    if debugging:
        print('Ps, Vs, As = ', Ps, Vs, As)
        print('Pe, Ve, Ae, Jmax = ', Pe, Ve, Ae, Jmax)
    xend, cend = ThreeSegmentSpline(Ps, Vs, As, Pe, Ve, Ae, deltaTimeEnd, debugging = debugging)
